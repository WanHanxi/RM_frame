...
{// 在这里调轮子的转速!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 //原版   CMFL.TargetAngle = (  ChassisSpeedRef.forward_back_ref	*0.075 
...
void controlLoop()
{
	...
	if(WorkState > 0)
	{
		...
		#ifdef CAN11
		for(int i=0;i<2;i++) if(can1[i]!=0) (can1[i]->Handle)(can1[i]);
		setCAN11();
		for(int i=2;i<4;i++) if(can1[i]!=0) (can1[i]->Handle)(can1[i]);
		setCAN11();
		#endif
		#ifdef CAN12

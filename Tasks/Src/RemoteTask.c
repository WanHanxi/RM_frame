...
{
	HAL_IWDG_Refresh(&hiwdg);    //读取到遥控数据后刷新看门狗一次（看门狗超时会导致复位）
	                             //未连接遥控器时可观测到此看门狗使GPIO抖动
	if(pData == NULL)
	{
			return;
	}
	...
	RC_CtrlData.rc.ch2 = (((uint16_t)pData[2] >> 6) | ((uint16_t)pData[3] << 2) |
											 ((uint16_t)pData[4] << 10)) & 0x07FF;
	...
	//拨轮
	RC_CtrlData.rc.dial = (((uint16_t)pData[16]) | ((uint16_t)pData[17])<<8);
